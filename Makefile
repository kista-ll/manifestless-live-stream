.PHONY: help bootstrap cert lint test build e2e run stream-start stream-stop stop clean phase0-check

PYTHON ?= C:/Users/y-aka/AppData/Local/Programs/Python/Python313/python.exe
NPM ?= C:/Program Files/nodejs/npm.cmd

help:
	@node -e "for (const line of ['help          Show available targets','bootstrap     Install dependencies and prepare local environment','cert          Generate localhost ECDSA P-256 certificate','run           Start WebTransport/API server, viewer, and SRT listener','stream-start  Start FFmpeg test encoder as SRT Caller','stream-stop   Stop FFmpeg test encoder','stop          Stop project server, viewer, segmenter, and test encoder','clean         Remove generated media, certificates, and test artifacts','lint          Run Ruff, mypy, and ESLint','test          Run pytest, Vitest, and segment smoke test','build         Compile server/viewer and verify generated segments when present','e2e           Run Playwright acceptance E2E']) console.log(line)"
